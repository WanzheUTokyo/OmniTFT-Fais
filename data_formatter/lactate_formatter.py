...
import data_formatter.base_formatter as base
...
class LactateFormatter(GenericDataFormatter):
    ...
    _column_definition = [
        ...
        ("lactate", DataTypes.REAL_VALUED,   InputTypes.TARGET),
    ]
    ...
    def split_data(self, df: pd.DataFrame):
        ...
        df["step_idx_2h"] = np.floor((df["hours_from_admit"] * 60.0) / BIN_MIN).astype(int)
        df = (
            df.sort_values(["subject_id", "charttime"])
              .drop_duplicates(subset=["subject_id", "step_idx_2h"], keep="last")
        )
        first_time = df.groupby("subject_id")["charttime"].transform("min")
        df["charttime"] = first_time + pd.to_timedelta(df["step_idx_2h"] * BIN_MIN, unit="m")
        df["hours_from_admit"] = (df["step_idx_2h"] * BIN_MIN) / 60.0

        df = df[df["hours_from_admit"] <= WINDOW_HOURS]
        if "itemid" in df.columns:
            df = df[~df["itemid"].isin(LAC_ITEM_IDS)]
        if "itemid" in df.columns:
            df = df[~df["itemid"].isin(LAC_ITEM_IDS)]
        ...
        for c, dt, _ in self._column_definition:
            ...
            if dt == DataTypes.REAL_VALUED and c not in ["subject_id", "lactate"]:
        ...
        for d in (train, valid, test):
            for aux in ("step_idx_2h", "orig_subject_id", "_win_start"):
                if aux in d.columns:
