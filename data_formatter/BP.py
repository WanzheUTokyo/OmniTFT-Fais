...
class BPFormatter(GenericDataFormatter):
    ...
    _column_definition = [
        ...
        # target
        ("blood_pressure", DataTypes.REAL_VALUED,   InputTypes.TARGET),
    ]
    ...
    def split_data(self, df: pd.DataFrame):
        ...
        if "itemid" in df.columns:
            df = df[~df["itemid"].isin(ALB_ITEM_IDS)]
        ...
        for c, dt, _ in self._column_definition:
            if c in df.columns:
                if dt == DataTypes.REAL_VALUED and c not in ["subject_id", "blood_pressure"]:
                    df[c] = df[c].fillna(-1)
    ...
    def _build_balanced_train_windows(self, df_train: pd.DataFrame) -> pd.DataFrame:
        ...
        for sid, g in df_train.groupby("subject_id", sort=False):
            ...
            for i in range(0, n - T + 1):
                ...
                if he > n:
                    break
                future = alb[hs:he]  # 长度 = HORIZON_STEPS
    ...
    def get_fixed_params(self):
        return {
            "total_time_steps": TOTAL_STEPS,     # = MAX_STEPS + HORIZON_STEPS
