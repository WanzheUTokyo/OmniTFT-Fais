...
class CreatinineFormatter(GenericDataFormatter):
    ...
    _column_definition = [
        ...
        ("creatinine", DataTypes.REAL_VALUED,   InputTypes.TARGET),
    ]
    ...
    def split_data(self, df: pd.DataFrame):
        ...
        if "itemid" in df.columns:
            df = df[~df["itemid"].isin(CRE_ITEM_IDS)]
        if "itemid" in df.columns:
            df = df[~df["itemid"].isin(CRE_ITEM_IDS)]
        ...
        for c, dt, _ in self._column_definition:
            ...
            if dt == DataTypes.REAL_VALUED and c not in ["subject_id", "creatinine"]:
