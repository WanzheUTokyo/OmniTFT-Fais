...
class SafeLabelEncoder(LabelEncoder):
    """LabelEncoder - 新类别→ -1（保持 classes_ 可扩展）"""
    def fit(self, y):
...
class HRFormatter(GenericDataFormatter):
    ...
    _column_definition = [
        ...
        ("heart_rate", DataTypes.REAL_VALUED,   InputTypes.TARGET),
    ]
    ...
    def split_data(self, df: pd.DataFrame):
        ...
        for c, dt, _ in self._column_definition:
            if c in df.columns:
                if dt == DataTypes.REAL_VALUED and c not in ["subject_id", "heart_rate"]:
                    df[c] = df[c].fillna(-1)
