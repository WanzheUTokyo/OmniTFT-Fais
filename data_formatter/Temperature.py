...
class TemperatureFormatter(GenericDataFormatter):
    ...
    _column_definition = [
        ...
        ("temperature", DataTypes.REAL_VALUED,   InputTypes.TARGET),
    ]
    ...
    def split_data(self, df: pd.DataFrame):
        ...
        for c, dt, _ in self._column_definition:
            if c in df.columns:
                if dt == DataTypes.REAL_VALUED and c not in ["subject_id", "temperature"]:
                    df[c] = df[c].fillna(-1)
